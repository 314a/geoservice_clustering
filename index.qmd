...
```{r}
d <- geoCH %>% group_by(provider) %>% summarise(layers = length(unique(name)), links = length(unique(endpoint)))
...
Anzahl eindeutige Layernamen nach provider `r `sum(d$layers)` und ein total an Datensätzen `r nrow(geoCH)`
...
ggplot(data = d, aes(x=layers, y= links)) + 
  ...
  geom_text_repel(mapping = aes(x = layers,y = links,label = provider),size = 2.4,
                  min.segment.length = 0,point.padding = 0.5,segment.color = "grey30", max.overlaps=13) + 
...
geoCH$metaKeywords <- gsub("(f|ht)tp(s?)://\\S+", "", geoCH$keywords, perl=T) # remove url
...
geoCH$metaKeywordsCountComma <- ifelse(geoCH$metaKeywordsCountComma>0,geoCH$metaKeywordsCountComma+1,geoCH$metaKeywordsCountComma)
...
dMeta <- geoCH %>% group_by(provider)  %>% summarise(
  ...
  abstractWordsMedian = median(metaAbstractWordCount[metaAbstractWordCount>0],na.rm=T),
...
dLong2 <- dMeta %>% pivot_longer(cols=c("abstractWordsMedian"  , "keywordsMedian" ,"keywordsURLListPerc",  "contactAtPerc"), names_to = "key", values_to = "values") %>% 
...
```{r}
#| fig-cap: Metadata usage in OGC Web Servics Switzerland, Median Wordcount for abstract and keywords, percentage of keyword entries that contain an url or an identifier, and percentage of email addresses provided in the contact fields for each provider
#| fig-height: 6
ggplot(data = dLong2, aes(x=provider, y= values)) + 
  ...
  facet_wrap(~factor(key, c("Abstract Wordcount Median", "Keywords Wordcount Median", "Keywords URL %", "Email %")),nrow=1, scales = "free_x")+
  labs(title = "Metadata", subtitle= "Usage of metadata fields in OWS",x = NULL,y = NULL,fill="",caption=caption)
