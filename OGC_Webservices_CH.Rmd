...
```{r}
# create bounding boxes from services
bboxstr <- "2740443.137 1232276.032 2765413.73 1258037.171 EPSG:2056"

str2BBox <- function(str){
  x <- unlist(strsplit(str ," "))
  epsg <- as.numeric(unlist(strsplit(x[5] ,":"))[2])
  x <- as.numeric(x[1:4])
  # create the geometry sfc column
  # with a lit of multiple boundingboxes
  # 
  ...
  p <- st_multipoint(rbind(c(x[1],x[2]),c(x[3],x[4])))
  bbox <- st_as_sfc(st_bbox(p)) 
  st_crs(bbox) <- epsg
  
  # create a bounding box class of a named vector
  ...
  pt1 = st_point(c(0,1))d = st_sf(data.frame(a=1:2, geom=sfc))
  
  # bbox <- st_bbox(c(xmin = as.numeric(x[1]), xmax = as.numeric(x[3]), ymax = as.numeric(x[4]), ymin = as.numeric(x[2])), crs = st_crs(epsg))
  return(bbox)
}
...
str2BBox <- function(bboxString){
  ...
  res <- lapply(bboxString,function(str){
    ...
    x <- unlist(strsplit(str ," "))
    x <- x[x != ""] # remove empty strings
    if(length(x)==0){return(st_polygon())} # return an empty polygon if coordinates are missing
    # epsg <- as.numeric(unlist(strsplit(x[5] ,":"))[2])
    x <- as.numeric(x[1:4])
    if(anyNA(x)){cat(i,": ",x,"has NA")}
    # create a bounding box polygon
...
bboxSF <- str2BBox(bboxstr)

y <- str2BBox(c(bboxstr,bboxstr))
ye <- str2EPSG(c(bboxstr,bboxstr))


d = st_sf(data.frame(a=1:2), geometry=y)


t <- geoCH[209,"BBOX"]

geoBBox <- str2BBox(geoCH[9780:9785,"BBOX"])
geoBBox <- str2BBox(geoCH[,"BBOX"])
geoEPSG <- str2EPSG(geoCH$BBOX)
...
geoCHSF <- st_sf(geoCH,geoBBox)
geoCHSF$epsg <- str2EPSG(geoCH$BBOX)
geoCHSF$isEmpty <- st_is_empty(geoCHSF)
